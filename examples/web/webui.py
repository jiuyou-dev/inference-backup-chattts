import os, sys

if sys.platform == "darwin":
    os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"

now_dir = os.getcwd()
sys.path.append(now_dir)

import argparse

import gradio as gr

from funcs import *
from ex import ex


def main():

    with gr.Blocks() as demo:
        gr.Markdown("# 🎙️ ChatTTS 语音合成 WebUI")
        gr.Markdown("- **GitHub 仓库**: https://github.com/2noise/ChatTTS")
        gr.Markdown("- **HuggingFace 仓库**: https://huggingface.co/2Noise/ChatTTS")

        with gr.Row():
            with gr.Column(scale=2):
                text_input = gr.Textbox(
                    label="📝 输入文本",
                    lines=4,
                    max_lines=4,
                    placeholder="请输入要合成语音的文本内容...",
                    value=ex[0][0],
                    interactive=True,
                )
                sample_text_input = gr.Textbox(
                    label="📄 参考文本（可选）",
                    lines=4,
                    max_lines=4,
                    placeholder="如果提供了参考音频和参考文本，Speaker Embedding 将被禁用。用于音色克隆。",
                    interactive=True,
                )
            with gr.Column():
                with gr.Tab(label="🎵 参考音频"):
                    sample_audio_input = gr.Audio(
                        value=None,
                        type="filepath",
                        interactive=True,
                        show_label=False,
                        waveform_options=gr.WaveformOptions(
                            sample_rate=24000,
                        ),
                        scale=1,
                    )
                with gr.Tab(label="🔑 参考音频代码"):
                    sample_audio_code_input = gr.Textbox(
                        lines=12,
                        max_lines=12,
                        show_label=False,
                        placeholder="上传参考音频后，在此处粘贴复制的代码，用于音色克隆。",
                        interactive=True,
                    )

        with gr.Row():
            refine_text_checkbox = gr.Checkbox(
                label="✨ 优化文本", value=ex[0][6], interactive=True
            )
            temperature_slider = gr.Slider(
                minimum=0.00001,
                maximum=1.0,
                step=0.00001,
                value=ex[0][1],
                label="🔥 音频温度（Temperature）",
                interactive=True,
            )
            top_p_slider = gr.Slider(
                minimum=0.1,
                maximum=0.9,
                step=0.05,
                value=ex[0][2],
                label="📊 top_P（核采样概率）",
                interactive=True,
            )
            top_k_slider = gr.Slider(
                minimum=1,
                maximum=20,
                step=1,
                value=ex[0][3],
                label="🔢 top_K（采样候选数）",
                interactive=True,
            )

        with gr.Row():
            gr.Markdown("### 🎛️ 文本优化控制符（Prompt Controls）")
        with gr.Row():
            oral_slider = gr.Slider(
                minimum=0,
                maximum=9,
                step=1,
                value=2,
                label="🗣️ 口语化程度（oral）",
                info="值越高 → 越多语气词、填充词、口语表达",
                interactive=True,
            )
            laugh_slider = gr.Slider(
                minimum=0,
                maximum=2,
                step=1,
                value=0,
                label="😂 笑声频率（laugh）",
                info="0=无笑声, 1=偶尔笑, 2=经常笑",
                interactive=True,
            )
            break_slider = gr.Slider(
                minimum=0,
                maximum=7,
                step=1,
                value=4,
                label="⏸️ 停顿密度（break）",
                info="值越高 → 越多自然停顿",
                interactive=True,
            )
            speed_slider = gr.Slider(
                minimum=1,
                maximum=9,
                step=1,
                value=5,
                label="⚡ 语速（speed）",
                info="5=正常, 1=很慢, 9=很快",
                interactive=True,
            )

        with gr.Row():
            voice_selection = gr.Dropdown(
                label="🎭 音色选择",
                choices=list(voices.keys()),
                value="默认音色",
                interactive=True,
            )
            audio_seed_input = gr.Number(
                value=ex[0][4],
                label="🎲 音频随机种子",
                interactive=True,
                minimum=seed_min,
                maximum=seed_max,
            )
            generate_audio_seed = gr.Button("\U0001f3b2", interactive=True)
            text_seed_input = gr.Number(
                value=ex[0][5],
                label="📌 文本随机种子",
                interactive=True,
                minimum=seed_min,
                maximum=seed_max,
            )
            generate_text_seed = gr.Button("\U0001f3b2", interactive=True)

        with gr.Row():
            spk_emb_text = gr.Textbox(
                label="🔊 说话人Embedding（音色特征）",
                max_lines=3,
                buttons=["copy"],
                interactive=True,
                scale=2,
            )
            dvae_coef_text = gr.Textbox(
                label="🎛️ DVAE系数（音频特征系数）",
                max_lines=3,
                buttons=["copy"],
                interactive=True,
                scale=2,
            )
            reload_chat_button = gr.Button("🔄 重新加载模型", scale=1, interactive=True)
            export_spk_button = gr.Button("💾 导出音色", scale=1, interactive=True)
            save_params_button = gr.Button("💾 保存参数", scale=1, interactive=True)
            load_params_file = gr.File(
                label="📂 加载参数文件",
                file_count="single",
                file_types=[".json"],
                scale=1,
                height=40,
            )
            load_params_button = gr.Button("📥 加载参数", scale=1, interactive=True)
            export_status = gr.Textbox(label="导出状态", interactive=False, scale=2, lines=1)

        with gr.Row():
            auto_play_checkbox = gr.Checkbox(
                label="▶️ 自动播放", value=False, scale=1, interactive=True
            )
            stream_mode_checkbox = gr.Checkbox(
                label="🌊 流式模式",
                value=False,
                scale=1,
                interactive=True,
            )
            split_batch_slider = gr.Slider(
                minimum=0,
                maximum=100,
                step=1,
                value=4,
                label="📦 批量分段数（Split Batch）",
                interactive=True,
            )
            generate_button = gr.Button(
                "🎤 开始生成", scale=2, variant="primary", interactive=True
            )
            interrupt_button = gr.Button(
                "⏹️ 停止",
                scale=2,
                variant="stop",
                visible=False,
                interactive=False,
            )

        text_output = gr.Textbox(
            label="📤 优化后的文本输出",
            interactive=False,
            buttons=["copy"],
        )

        sample_audio_input.change(
            fn=on_upload_sample_audio,
            inputs=sample_audio_input,
            outputs=sample_audio_code_input,
        ).then(fn=lambda: gr.Info("参考音频代码已在另一个标签页中生成。"))

        # 使用Gradio的回调功能来更新数值输入框
        voice_selection.change(
            fn=on_voice_change, inputs=voice_selection, outputs=audio_seed_input
        )

        generate_audio_seed.click(generate_seed, outputs=audio_seed_input)

        generate_text_seed.click(generate_seed, outputs=text_seed_input)

        audio_seed_input.change(
            on_audio_seed_change, inputs=audio_seed_input, outputs=spk_emb_text
        )

        reload_chat_button.click(
            reload_chat, inputs=dvae_coef_text, outputs=dvae_coef_text
        )

        interrupt_button.click(interrupt_generate)

        export_spk_button.click(
            fn=export_speaker_to_file,
            inputs=spk_emb_text,
            outputs=export_status,
        )

        save_params_button.click(
            fn=save_inference_params,
            inputs=[
                text_input,
                temperature_slider,
                top_p_slider,
                top_k_slider,
                oral_slider,
                laugh_slider,
                break_slider,
                speed_slider,
                refine_text_checkbox,
                audio_seed_input,
                text_seed_input,
                split_batch_slider,
                stream_mode_checkbox,
                spk_emb_text,
            ],
            outputs=export_status,
        )

        load_params_button.click(
            fn=load_inference_params_from_file,
            inputs=load_params_file,
            outputs=[
                text_input,
                temperature_slider,
                top_p_slider,
                top_k_slider,
                oral_slider,
                laugh_slider,
                break_slider,
                speed_slider,
                refine_text_checkbox,
                audio_seed_input,
                text_seed_input,
                stream_mode_checkbox,
                split_batch_slider,
                spk_emb_text,
                dvae_coef_text,
            ],
        )

        @gr.render(inputs=[auto_play_checkbox, stream_mode_checkbox])
        def make_audio(autoplay, stream):
            audio_output = gr.Audio(
                label="🔊 生成的音频",
                value=None,
                format="mp3" if use_mp3 and not stream else "wav",
                autoplay=autoplay,
                streaming=stream,
                interactive=False,
                show_label=True,
                waveform_options=gr.WaveformOptions(
                    sample_rate=24000,
                ),
            )
            generate_button.click(
                fn=set_buttons_before_generate,
                inputs=[generate_button, interrupt_button],
                outputs=[generate_button, interrupt_button],
            ).then(
                refine_text,
                inputs=[
                    text_input,
                    text_seed_input,
                    refine_text_checkbox,
                    temperature_slider,
                    top_p_slider,
                    top_k_slider,
                    split_batch_slider,
                    oral_slider,
                    laugh_slider,
                    break_slider,
                    speed_slider,
                ],
                outputs=text_output,
            ).then(
                generate_audio,
                inputs=[
                    text_output,
                    temperature_slider,
                    top_p_slider,
                    top_k_slider,
                    spk_emb_text,
                    stream_mode_checkbox,
                    audio_seed_input,
                    sample_text_input,
                    sample_audio_code_input,
                    split_batch_slider,
                    oral_slider,
                    laugh_slider,
                    break_slider,
                    speed_slider,
                ],
                outputs=audio_output,
            ).then(
                fn=set_buttons_after_generate,
                inputs=[generate_button, interrupt_button, audio_output],
                outputs=[generate_button, interrupt_button],
            )

        gr.Examples(
            examples=ex,
            inputs=[
                text_input,
                temperature_slider,
                top_p_slider,
                top_k_slider,
                audio_seed_input,
                text_seed_input,
                refine_text_checkbox,
            ],
        )

    parser = argparse.ArgumentParser(description="ChatTTS demo Launch")
    parser.add_argument(
        "--server_name", type=str, default="0.0.0.0", help="server name"
    )
    parser.add_argument("--server_port", type=int, default=8080, help="server port")
    parser.add_argument("--root_path", type=str, help="root path")
    parser.add_argument("--custom_path", type=str, help="custom model path")
    parser.add_argument("--coef", type=str, help="custom dvae coefficient")
    parser.add_argument(
        "--disable_cache", action="store_true", help="enable model cache"
    )
    args = parser.parse_args()

    logger.info("loading ChatTTS model...")

    if load_chat(args.custom_path, args.coef, not args.disable_cache):
        logger.info("Models loaded successfully.")
    else:
        logger.error("Models load failed.")
        sys.exit(1)

    spk_emb_text.value = on_audio_seed_change(audio_seed_input.value)
    dvae_coef_text.value = chat.coef

    demo.launch(
        server_name=args.server_name,
        server_port=args.server_port,
        root_path=args.root_path,
        inbrowser=True,
        footer_links=["api", "gradio", "settings"],
    )


if __name__ == "__main__":
    main()
